...
function layoutEdge(edge) {
    ...
    if(edge.type == "curve") {
        var points = [sourcePoint, subtractVectors(sourcePoint, {"x" : 10,"y" : 100}), subtractVectors(targetVec, {"x" : 0, "y" : 150}), addVectors(targetVec, makeVector(10,-10))]
...
function contractStep1(path,duration) {
    ...
    n18.transition().duration(duration).attr({"x" : 760, "y" : 240})
    n11.transition().duration(duration).attr({"cx" : 620, "cy" : 140})
    n12.transition().duration(duration).attr({"cx" : 680, "cy" : 140})
    n13.transition().duration(duration).attr({"cx" : 740, "cy" : 140})
    n14.transition().duration(duration).attr({"cx" : 800, "cy" : 140})
    n15.transition().duration(duration).attr({"cx" : 860, "cy" : 140})

    e10.transition().duration(duration).attr({"x2" : 610.0153964679458, "y2" : 146.65640235470275})

    e12.transition().duration(duration).attr({"x1": 632, "y1": 140 , "x2" : 674, "y2": 140})
    e13.transition().duration(duration).attr({"x1": 686, "y1": 140 , "x2" : 728, "y2": 140})
    e14.transition().duration(duration).attr({"x1": 752, "y1": 140 , "x2" : 800, "y2": 140})
    e15.transition().duration(duration).attr({"x1": 800, "y1": 140 , "x2" : 848, "y2": 140})

    e17.transition().duration(duration).attr({"x2" : 760, "y2": 260})

    var p1 = makeVector(400, 180)
    var p2 = makeVector(380, 80)
    var p3 = makeVector(860, 40)
    var p4 = makeVector(870, 130)
    e22.transition().duration(duration).attr({"d" : lineGenerator([p1, p2, p3, p4])})

    // node.x - 20,
    // node.y + 10,
    //  {"name": "d3", "type": "delta", "latex": "\\delta^l_o", "x": 700, "y": 300},
    //  {"name": "d4", "type": "delta", "latex": "\\delta^n_p", "x": 560, "y": 180}

    delta3.transition().duration(duration).attr({"x": 540, "y": 190})
    delta4.transition().duration(duration).attr({"x": 680, "y": 310})
    // var newD3_latex_raw = "\\delta^n_o";
    // var newD3_latex_query = encodeURI(newD3_latex_raw);
    //
    // var newD4_latex_raw = "\\delta^l_p";
    // var newD4_latex_query = encodeURI(newD4_latex_raw);
    // setTimeout(function () {
    //     delta3.transition().duration(duration).select("img").attr({"src": latex_render_url + newD3_latex_query})
    //     delta4.transition().duration(duration).select("img").attr({"src": latex_render_url + newD4_latex_query})
    // }, duration - 200)
...
function contractStep3(path, duration) {
    ...
    // 7, 8, 9, 10, 11, 16 (is low)
    var delta4 = d3.select("#d4") // to move
    var delta2 = d3.select("#d2") // to merge
    var delta3 = d3.select("#d3") // to merge
    ...
    // move the rightmost delta a bit to the left
    delta4.transition().duration(duration).attr({"x" : delta4.attr("x") - moveVector.x, "y" : delta4.attr("y") - moveVector.y})

    // merge delta2 and delta3
    delta3.transition().duration(duration).attr({"x": delta2.attr("x"), "y": delta2.attr("y")}).remove()
    delta2.transition().duration(duration).remove()

    // create new delta with the same id as delta2
    var latex_raw = "\\delta^{l,m}_{o,q}";
    var latex_query = encodeURI(latex_raw);
    setTimeout(function() {
...
function contractStep4(path, duration) {
    ...
    delta4.transition().duration(duration).attr({"x" : n7.attr("cx"), "y" : n7.attr("cy") - 15}).remove()
    ...
    var latex_raw = "\\mathbf{vrouwen}_n \\mathbf{haten}_{onq}";
    var latex_query = encodeURI(latex_raw);
    setTimeout(function() {
        d3.select("svg").append("foreignObject")
         .attr({
...
function contractStep5(path, duration) {
    ...
    var latex_raw = "\\mathbf{die}_{jklm} \\mathbf{vrouwen}_n \\mathbf{haten}_{lnm}";
    var latex_query = encodeURI(latex_raw);
    setTimeout(function() {
        d3.select("svg").append("foreignObject")
         .attr({
...
function contractStep6(path, duration) {
    var n1 = d3.select("#n1")
    ...
    var latex_raw = "\\mathbf{mannen}_i \\mathbf{die}_{iklm} \\mathbf{vrouwen}_n \\mathbf{haten}_{lnm}";
    var latex_query = encodeURI(latex_raw);
    setTimeout(function() {
        d3.select("svg").append("foreignObject")
         .attr({
...
function moveNodes6(dur) {
    d3.select("svg").call(contractStep6, dur)
}
